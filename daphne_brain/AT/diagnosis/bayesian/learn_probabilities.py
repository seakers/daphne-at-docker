...
# probabilities with those input by hand <-- note this has not been updated for the new hatch
# status node
# prior_cpds_dict = {}
...
# Below command works if running script from the daphne-at-docker folder
...
estimator = BayesianEstimator(new_model, samples)
...
learned_cpds = estimator.get_parameters(
    ...
    pseudo_counts=pseudo_counts # can increase if CPDs still appear uniform
)

for cpd in learned_cpds:
    print(cpd)

cpds_filepath = os.path.join(current_dir, 'all_cpds.txt')
